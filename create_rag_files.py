...
def anonymize_csv(input_file, output_file):
    with open(input_file, "r", newline="") as infile, open(
        ...
        fieldnames = [
            ...
            if field not in ["Website", "Email Address"]
        ]
        ...
        name_column = next(
            (field for field in fieldnames if "name" in field.lower()), None
        )

        if not name_column:
            print(
                "Warning: No 'Name' column found. Proceeding without name anonymization."
            )
        ...
        for row in reader:
            ...
            if name_column:
                ...
                if len(name_parts) > 1:
                    row[name_column] = (
                        f"{' '.join(name_parts[:-1])} {name_parts[-1][0]}"
                    )
